...
/// <summary>
/// Serves filtered torrent counts, holding each until a mutation invalidates it.
/// </summary>
/// <remarks>
...
public sealed class TorrentCountCache : IDisposable
{
    ...
    public void Dispose()
    {
        ...
        // (process shutdown). A fire-and-forget mutation could still race Invalidate here, but the
        // window is process-exit only and CancellationTokenSource.Dispose is idempotent, so the worst
        // case is a swallowed ObjectDisposedException from Cancel as the process terminates. If this
        // cache ever becomes non-singleton (disposed during normal operation while mutations run),
        // add a guard: mutual exclusion between Invalidate and Dispose (and the cache Set path),
        // keeping the GetOrAddAsync TryGetValue fast path lock-free.
