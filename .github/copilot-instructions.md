...
**Comment only what needs it.** Write none. Add one only when you can name the question a reader would have that the code does not answer — then write the answer. Examples of what qualifies: an ordering or concurrency rationale, a measured surprise, a maintainer warning — none of which a test can express. A comment that could be an assertion should be one instead: both say the same thing, but only the test fails when it stops being true. Before keeping one, delete it: if nothing is lost that the name, signature and body give up in seconds, leave it deleted. This decides **whether** to comment; publicly used members are the exception.
