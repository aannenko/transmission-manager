...
**Torrent source options are validated by hand, not by data annotations.** Each options class has exactly one `IValidateOptions<T>` beside it, named `Validate{OptionsClass}`, and no validation attributes at all — every options property instead carries a `<remarks>` pointing at that validator, which owns the bounds and the reasons. This is not stylistic: the checks are *ordered* (a pattern is compiled with `RegexMatchTimeout`, so an out-of-range timeout must be caught before the compile, or `new Regex` throws `ArgumentOutOfRangeException` and startup reports that instead of the setting at fault), and the options system runs every registered validator and merges their failures, so a second validator cannot be made to run after the first. The cost is that a new property is validated only if you remember; each validator therefore has a test class with one case per setting.
...
**Comment only what needs it.** A comment earns its place by making unclear code clear, or by carrying what the code cannot say — an invariant, an ordering or concurrency rationale, a throw condition, a measured surprise, a maintainer warning. Most members need none, public ones included; a `<summary>` restating the signature is noise.

**Shape follows visibility.** A publicly accessible member leads with `<summary>`, never `<remarks>` alone, and adds `<param>` / `<returns>` / `<exception>` / `<typeparam>` where each says something the signature does not — `ServiceProviderTorrentSourceExtensions.FindMagnetUriAsync` is the worked full form. Non-public members may be `<remarks>`-only. A test's three-part name is already its summary, so a test takes `<remarks>` alone, for why it exists.

**Short, active, and about this member only.** Lead with what the member does or asserts — `ProblemDetailsKeys` is the worked terse form ("Blames the torrent's source for not yielding a magnet link"). Do not explain the mechanics of other code that merely reaches this one: that belongs where it lives, and the copy here goes stale when it changes (a remark describing which keys an empty update was refused under outlived that behaviour inside a single change set). Keep such a statement only when it is genuinely useful and has nowhere else to sit.
